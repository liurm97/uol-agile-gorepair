...
export const ServiceCard = ({
  ...
  useEffect(() => {
    firebaseObject
      .retrieveSpecificBookingServices(category, subcategory)
      .then((result) => {
        setServices([...result]);
      });
  }, []);
        ...
        return (
          <HStack key={uuid}>
            ...
            <Button
              ...
              size="xs"
              onClick={() => {
                decreaseOrder(uuid, item);
                setSelectedServices(
                  selectedServices,
                  item,
                  Number(document.getElementById(uuid)?.innerHTML)
                );
              }}
            >
              {" "}
            ...
            <Text key={uuid + 2} as="span" id={uuid}>
              0
            </Text>
            <Button
              key={uuid + 3}
